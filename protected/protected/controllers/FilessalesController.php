...
class FilessalesController extends Controller
...
where $where "; 
    ...
    $res=array();
			...
			echo '<tr><td>'.$key['plotno'].'</td><td>'.$key['project_name'].'</td><td><a href="'.$home.'/index.php/user/filehistory?id='.$key['id'].'">'.$key['plot_detail_address'].'</a></td><td>'.$key['plot_size'].'</td><td>'.$key['size'].'</td><td>'.$key['com_res'].'</td><td>'.$key['sector_name'].'</td><td>'.$key['street'].'</td>';
				echo '<td>';
...
$F='';
			...
			}elseif($key['status']=='Requested'){if(!empty($key['fstatus'])){$M='M';}else{$F='F'; } 
			echo'<a href="'.$home.'/index.php/memberplot/requested_detail?id='.$key['id'].'">' ."Requested".'('.$M.$F.')'.'</a>';
