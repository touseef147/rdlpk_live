...
public function actionAjaxplot($sec,$size2,$ptype)
	{	

	$connection = Yii::app()->db;  

		 $sql_street  = "SELECT * from plots 
		 
		 where street_id='".$sec."' and type ='Plot' and status='' and size2='".$size2."' and ctag='' and com_res='".$ptype."'";
	   ...
	   $connection = Yii::app()->db;  
		if(Yii::app()->session['user_array']['per4']=='1')
		  ...
		  $error =array();
			...
			if(empty($error)){
			$map="UPDATE dmap SET plot_id='".$_POST['id']."' where plot_id='".$_POST['plot_detail_address']."'";
				$command = $connection -> createCommand($map);
               $command -> execute();	
...
if($_POST['stat']==5){$where.="and plots.status ='Requested(T)'";}
if($_POST['stat']==6){$where.="and plots.atype='Against Land'";}
...
if($key['status']==''){ 
			...
			<li role="presentation"><a href="updatefile?id='.$key['id'].'&&project_id='.$key['project_id'].'">Edit</a></li>
...
if(x == true){
window.location="deletefile?id=" + id + "&&did=" + idd + "";
}
	...
	{      $connection = Yii::app()->db;  
        ...
        if(empty($error))

			{
			   ...
			   $sql="UPDATE plots set project_id='".$_POST['project_id']."',PLcharges='".$_POST['PLcharges']."',remarks='".$_POST['remarks']."',street_id='".$_POST['street_id']."',plot_detail_address='".$_POST['plot_detail_address']."',plot_size='".$_POST['plot_size']."',size2='".$_POST['size2']."',price='".$_POST['price']."',create_date='".date('Y-m-d h:i:s')."',com_res='".$_POST['com_res']."',sector='".$_POST['sector']."',cstatus='".$_POST['cstatus']."' where id='".$_POST['id']."' ";  
...
$connection = Yii::app()->db;  
	...
	if ((empty($result_data))){
			...
			$query  = 'DELETE from  plots  where id='.$_REQUEST['id'].' ';
		   $command = $connection -> createCommand($query);
            $command -> execute();
