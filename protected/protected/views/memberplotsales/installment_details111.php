...
<tr><td>Prime Location Charges</td><td style="text-align:right"><?php echo floatval($row['PLcharges']);?></td> </tr>
...
$sql_memberas= "SELECT * FROM memberplot mp
		...
		$result_membersas = $connection->createCommand($sql_memberas)->queryRow();
if($result_membersas['status']=='Sales' ){?>
...
$gtotalsur=$gtotalsur+$totalduesur;
     ...
     <td>'.$pay['remarks'].'</td>
     <td>';if(!empty($pay['fstatus'])){if($pay['fstatus']=='approved'){ echo '<b style="color:Green;">Verified</b>';}else{ echo '<b style="color:red;">'.$pay['fstatus'].'</b>';}} else{}
if($result_membersas['status']=='Sales' ){echo '<a href="deleteins?did='.$pay['id'].'&id='.$pay['plot_id'].'">Delete</a>';}
	 $id='';
	$id=$pay['id'];
...
';*/
if($due==0){$due=1;}
if(empty($oins)){$oins=0;}else{ echo '<tr><td><b>Outstanding Installment</b></td><td></td><td></td><td style="text-align:right;"><b>'.number_format($oins).'</b></td>
<td style="text-align:right;"><b>'.number_format(($oins)/$due*100).'%'.'</b></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
...
<section class="reg-section " style="font-size:11px;">
 <table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td width="70%"><h4>Plot Details</h4>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
<?php $res=array();
    foreach($info as $row){
$old_date = $row['create_date'];            
...
<?php
	if($row['stst']==2){echo'<span style="color:red">Blocked</span>';}else{echo $row['plot_detail_address'];
	//echo '<b>Date  :</b>' .$new_date.'</br>';
  ...
  <?php
    ...
    <td width="30%"><h4>Member Details</h4>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
	<?php $res=array();
    foreach($members as $mem){             
?>  <tr>
...
<td style="text-align:right"><b>';if(empty($discountr['details'])){ echo'';}else{ echo $discountr['details'];} echo '</b></td>
</tr>';
if($due==0){$due=1;}
...
<input style="float:left;" type="submit" name="submit" value="Print Report" /></form><div class="span5">
