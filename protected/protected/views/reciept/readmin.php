...
$member  = "SELECT * from members where cnic='".$data['cnic']."' ";
$member_res = $connection->createCommand($member)->queryRow();
?>
     <div class="float-left">
    <p class="reg-left-text"> Name:</p><p><?php echo $member_res['name'] ?></p>
    </div>
 <div class="float-left">
  ...
  <div class="float-left">
    <p class="reg-left-text" >Date:</p><p><?php echo $data['date'] ?></p>
  </div>
  ...
  $result_payments1 = $connection->createCommand($sql_payment1)->queryAll();
   ...
   $style='';
   if($rem>0){$style='background-color:Green;';}else{$style='background-color:Red;';}
   ?>
<?php 
$connection = Yii::app()->db; 
$sql_plot12  = "
...
<?php $da=''; 
if($data['filed']==1){$da='<option value="1">Filed</option>';}
elseif($data['filed']==2){$da='<option value="2">Hold</option>';}else{$da='<option value=""> Select Status</option>';};?>
