...
<div class="shadow">
  <h3>Assign User/Sales Center</h3>
  <span><a style="float:right;" href="unsupervised_list" class="btn">Back to list</a></span>
<?php 
...
<tbody>
    ...
    <tr>
<td>
<select  name="sc_id" id="sc_id" style="width:300px;">
<option value="">Select User</option>
				<?php                
             $sql_sales = "SELECT * FROM sales_center";
			 $result_sales = $connection->createCommand($sql_sales)->queryAll();
...
<option value="">Select User</option>
<?php                
             $sql = "SELECT * FROM user where per18 ='1'  AND status=1";
			$result_data = $connection->createCommand($sql)->queryAll();
				foreach($result_data as $user){
				echo '<option value="'.$user['id'].'">'.$user['firstname'].'&nbsp;'.$user['middelname'].'&nbsp;'.$user['lastname'].'</option>';
						}
?>
</select></td> </tr>  
