...
$new_date = date('d-m-Y', $middle); 
   ...
   if($row['type']=='file'){echo '<b>File Size&nbsp;&nbsp;&nbsp;:</b>';}else {echo '<b>Plot Size  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:</b>';} echo $row['size'].' ('.$row['plot_size'].')'.'</br>';
...
$i++;
  ...
  echo '<tr><td>'.$pay['lab']. '</td>
     ...
     <td>'.$pay['due_date'].'</td>
     <td>'.$pay['paid_date'].'</td>
     <td style="text-align:right">'.$pay['dueamount'].'</td>
     ...
     <td>'.$pay['remarks'].'</td>
     <td>';if(!empty($pay['fstatus'])){ echo'<b style="color:Green;">Verified</b>';} else{ echo'';}} 
	if($due==0){$due=1;}
...
';
	echo
'<tr><td><b>Outstanding Installment</b></td><td><b>'.number_format($oins-$discountr['discount']).'</b></td><td><b>'.number_format(($oins-$discountr['discount'])/$due*100).'%'.'</b></td></tr>
