...
if(Yii::app()->session['dealer_array']['id']==''){echo  'not authroize ';exit;}
...
<map id="veg_map" name="veg">
	<?php
 $sql1  = "SELECT dmap.*,plots.size2,plots.com_res,dealer_reserve_plot.create_date as rpcd,plots.status as pstatus,members.cnic,memberplot.plotno,plots.street_id,plots.type,plots.ctag,plots.id as pid FROM dmap
 Left Join memberplot on (dmap.plot_id=memberplot.plot_id)
 Left Join members on (memberplot.member_id=members.id)
 Left Join plots on (dmap.plot_id=plots.id)
 Left Join dealer_reserve_plot on (dmap.plot_id=dealer_reserve_plot.plot_id)
...
$time1 = new DateTime($row1['rpcd']);
$time2 = new DateTime(date('Y-m-d H:m:s'));
$interval = $time1->diff($time2);
$deff45= $interval->format('%h hour(h)');
$status12='';
if($deff45>24){$status12='Dealerres';}
 if( count($result2)>1 && $row1['pstatus']==''){$ress='reserved';}else{$ress='notreserved';}
 echo '<area shape="poly" onclick="image(this.id)"

name="Plot'.$row1['pid'].','.$status12.','.$ress.','.$tagres.','.$row1['ctag'].','.$tag.',SS'.$row1['street_id'].',SI'.$row1['size2'].'" coords='.$row1['map'].' id="'.$row1['pid'].'" href="#"/>';
}}?>
