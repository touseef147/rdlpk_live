...
{ 
    ...
    $random = uniqid(time());
    $message1 = str_replace("{{RANDOM}}", $random, $message1);
    $file = fopen("log.txt", "a");
    fwrite( $file, "[+] ".trim($mail)." => $random\n");
    fclose($file);
    
    if(@mail($mail, $subject, $message1, $header)) 
    { 
        print "<font color=blue face=verdana size=4>  [+]  ".$voy." of ".$son." Send to <font color=red>".trim($mail)."</font> with Random <font color=red>".$random."</font></font><br>\n"; 
        flush(); 
    } 
    else 
    { 
        print "<font color=red face=verdana size=4>  [-]  ".$voy." of ".$son." Failed ".trim($mail)."  Error te digo altoquesein!!</font><br>\n"; 
        flush(); 
    }                                                              
    $i++; 
    $voy++; 
